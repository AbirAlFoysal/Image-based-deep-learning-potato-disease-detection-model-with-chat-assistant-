...
MODEL_DIR = os.path.join(os.path.dirname(__file__), '..', 'detection_service', 'tuber_model')
TUBER_MODEL_PATH = os.path.join(MODEL_DIR, 'resnet_potato_tuber_model.onnx')
...
def _normalize_class_name(class_name):
    cleaned_name = class_name.replace('Augmented ', '').replace('Potato___', '')
    cleaned_name = cleaned_name.replace('_', ' ').strip()

    if cleaned_name.startswith('Potato '):
        cleaned_name = cleaned_name[len('Potato '):]

    if cleaned_name.endswith(' Disease'):
        cleaned_name = cleaned_name[:-len(' Disease')]

    if cleaned_name == 'Healthy Potato':
        return 'Healthy'
...
def predict_tuber_disease(image: Image.Image, top_k=3):
    ...
    for idx in top_indices:
        predictions.append({
            'class': _normalize_class_name(tuber_model.class_names[idx]),
            'raw_class': tuber_model.class_names[idx],
    ...
    return {
        ...
        'is_healthy': predictions[0]['class'] == 'Healthy'
    }
